...
; ★ GL_Process_LoadScripts：启动阶段扫描所有可管理的子脚本。
...
GL_Process_LoadScripts(glLoadScriptsFolder) {
    ...
    return GL_Process_SortScripts(glLoadScriptsList)
}

; ★ GL_Process_SortScripts：只负责排序，不负责启动、不负责 GUI。
; ★ 以后新增脚本时不需要在这里登记名称；只要目录中有 MAIN.ahk，管理器就会自动发现。
GL_Process_SortScripts(glSortScriptsList) {
