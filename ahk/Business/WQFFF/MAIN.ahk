...
; ★ → MAIN 初始化 GUI、运行状态、F 间隔、定时器、HWND 文件、输入保护
; ★ → 注册 *F6 / *F7
; ★ → 注册 GL 的 0xB001 退出消息
...
MAIN() {
    ...
    wqfffMainPressTimer := (*) => Task_Process_PressFTimer(&wqfffMainRunning)

    ; ★ 控制键必须加入忽略列表，避免 F6/F7 被输入保护误判成用户干扰。
    wqfffMainControlKeys := ["F6", "F7"]
    wqfffMainInputGuard := Input_InputControl_CreateGuard(wqfffMainControlKeys)

    ; ★ 写入本脚本 GUI HWND，供 GL 管理器发送退出消息。
    Task_Process_WriteHwnd(wqfffMainHwndFile, wqfffMainGui)

    ; ★ F6 → Process：启动 W/Q 按住和 F 循环。
    Hotkey("*F6", (*) => Task_Process_Start(&wqfffMainRunning, wqfffMainFInterval, wqfffMainPressTimer, wqfffMainStatusText, wqfffMainInputGuard))

    ; ★ F7 → Process：停止 W/Q 按住和 F 循环。
    Hotkey("*F7", (*) => Task_Process_Stop(&wqfffMainRunning, wqfffMainPressTimer, wqfffMainStatusText, wqfffMainInputGuard))

    ; ★ GL 管理器发送 0xB001 后进入这里；wParam 是退出原因。
    OnMessage(0xB001, (wParam, lParam, msg, hwnd) => Task_Process_Exit(wParam, wqfffMainHwndFile, &wqfffMainRunning, wqfffMainPressTimer, wqfffMainInputGuard, wqfffMainStatusText))

    ; ★ 无论正常还是异常退出，都停止 Timer 并释放 W/Q/F。
    OnExit((*) => Task_Action_ReleaseKeys(&wqfffMainRunning, wqfffMainPressTimer, wqfffMainInputGuard))
}
