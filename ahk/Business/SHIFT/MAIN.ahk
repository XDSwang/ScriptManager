...
; ★ → MAIN 初始化 GUI、运行状态、HWND 文件、输入保护
; ★ → 注册 *F6 / *F7
; ★ → 注册 GL 的 0xB001 退出消息
...
MAIN() {
    ...
    Task_Process_WriteHwnd(shiftMainHwndFile, shiftMainGui)

    ; ★ F6 → Process：启动业务流程。
    Hotkey("*F6", (*) => Task_Process_Start(&shiftMainHeld, shiftMainStatusText, shiftMainInputGuard))

    ; ★ F7 → Process：停止业务流程。
    Hotkey("*F7", (*) => Task_Process_Stop(&shiftMainHeld, shiftMainStatusText, shiftMainInputGuard))

    ; ★ GL 管理器发送 0xB001 后进入这里；wParam 是 GL 提供的退出原因。
    OnMessage(0xB001, (wParam, lParam, msg, hwnd) => Task_Process_Exit(wParam, shiftMainHwndFile, &shiftMainHeld, shiftMainInputGuard, shiftMainStatusText))

    ; ★ 无论正常退出还是异常退出，都最终释放 Shift，避免按键卡住。
    OnExit((*) => Task_Action_ReleaseKeys(&shiftMainHeld, shiftMainInputGuard))
}
